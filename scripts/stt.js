...
var stts=false;
var btnRecord = document.getElementById('recordButton');
...
btnRecord.addEventListener("click", function(event) {
	btnRecord.classList.add("animate-flicker");
	btnStop.classList.add("bot-dois");
	btnStop.classList.remove("invisible");
	btnStop.classList.remove("btn-chat");
...
btnStop.addEventListener("click", function(event) {
	...
	btnRecord.classList.remove("animate-flicker");
	event.preventDefault();
	stts=true;
//	btnRecord.disabled = false;
//	btnPause.disabled = true;
	...
	xhr.addEventListener("load", function() {
		if(xhr.status == 200) {
			...
			resposta[0].alternatives.forEach(function(transcript) {
				...
				callBot(transcript.transcript);//mensagem 
